/* scripts/keepers/metrics-keeper.ts
   ...
   ENV / CLI:
     ...
     REFRESH_MS            (default 60000)

   Usage:
     ts-node scripts/keepers/metrics-keeper.ts \
       --endex 0x... \
...
function parseArgs(): Opts {
  ...
  const refreshMs = Number(arg.get("refreshMs") || process.env.REFRESH_MS || 60_000);
...
type Position = {
  id: bigint;
  isLong: boolean;
  status: number;           // 0=Open,1=AwaitingSettlement,2=Liquidated,3=Closed  (adjust to your enum)
  collateral: bigint;       // USDC 6d
  entryPriceE8: bigint;     // price 8d
  settlementPriceE8: bigint;// price 8d (if closed)
  closeCause: number;       // enum for cause
};
  ...
  async function collect() {
    ...
    for (let id = 1n; id < nextId; id++) {
      try {
        ...
        const pos: Position = {
          id,
          isLong: Boolean(p.isLong ?? p[0]),
          status: Number(p.status ?? p[1]),
          collateral: BigInt(p.collateral ?? p[2]),
          entryPriceE8: BigInt(p.entryPriceE8 ?? p[3]),
          settlementPriceE8: BigInt(p.settlementPriceE8 ?? p[4]),
          closeCause: Number(p.closeCause ?? p[5]),
        };
        ...
        switch (pos.status) {
          case 0: open++; break;
          case 1: awaiting++; break;
          case 2: liquidated++; break;
          case 3:
            closed++;
